/*    mapmanager.c   
	- compile with  gcc -g mapmanager.c initmaps.c mapmisc.c ../ozdisplay/SDL2init.c initgps.c -o ../../mapmanager -lSDL2 -lSDL2_image -lm
...
//	static PMTmapmapindex*		homemapindex;
	...
	{	
		...
		while( TRUE )
		{
			...
			if( (mapindexTemp = findgpsmap( gps, mapsetNow) ) == NULL ) 
  			{	// not found
				mapset++;
				if(bookend == 9999)
				{
			...
			{	//found 
    			...
    			homemap = SDL_CreateRGBSurface(0, mapNow->w, mapNow->h, 32,
	...
	else if( mapflags == GPSMAP  && !firsttime )  /* return saved homemap */
	{
    	...
    	mapNow = SDL_CreateRGBSurface(0, homemap->w, homemap->h, 32,
                0x000000ff, 0x0000ff00, 0x00ff0000, 0xff000000); 
