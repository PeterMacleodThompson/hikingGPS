...
PMTmapset* mapsetInit( int mapsetcount )		
{
    ...
    while( fgets( inbuf, 400, fp) != NULL )
    {
		...
		sscanf( inbuf, "%s %s %d %d", 
			(mapsetPtr+i)->name, (mapsetPtr+i)->usage,
    ...
    (mapsetPtr+i-1)->sequence = 9999;  //closing bookend marking last mapset record
	close(fp);
    return( mapsetPtr );
}
...
void mapindexInit(PMTmapset mapset[], int mapsetcount)
{
    ...
    for( i=0; i<mapsetcount; i++)		// for each mapset
    {
    	...
    	while( fgets( inbuf, 400, fp) != NULL )
		{
			if(inbuf[0] == '#')
				continue;		/* skip this record */
	    	sscanf( inbuf, "%d %d %d %s", 
				&mapkey, &mapwidth, &mapheight, mapFilename
		    	);
