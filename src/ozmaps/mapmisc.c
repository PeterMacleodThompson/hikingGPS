...
PMTmapindex* findmapkey( int mapkey, PMTmapset* mapsetNow, PMTmapindex* mapindexNow )
{
    ...
    // each map -17 to +100 from where binary seach stopped,  is checked for a match
    while( low <= high)
    {	/* if latitudes match  && longitude bigger ==> found */
	...
	printf("NOT Found mapkey %d mapset %d %s\n", 
		mapkey, mapsetNow->sequence, mapsetNow->name); 
...
/*******  PMTmapindex* findgpsmap( uint64_t gps, PMTmapset mapsetNow ) ************
	...
	- current mapset ie mapsetNow
		if mapsetNow is NULL, find any map in any mapset starting at sequence=0
		if mapsetNow points to a mapset, look in this mapset only.  Return
...
PMTmapindex *findgpsmap( uint64_t gps, PMTmapset* mapsetNow )
{
    ...
    while (low <= high)
    {
		...
		else 
		{
			printf("Bin Srch Found GPS map %s mapkey %d mapset %d %s GPS %llu\n", 
				mid->mapname, mid->mapkey, mapsetNow->sequence, mapsetNow->name, gps); 
	    	return (mid);	/* found */
		}
    }
    ...
    while( low <= high)
    {
    	if( inthemap( gpskey, low) )
		{
			printf("Lin Srch Found GPS map %s mapkey %d mapset %d %s GPS %llu\n", 
				low->mapname, low->mapkey, mapsetNow->sequence, mapsetNow->name, gps); 
	    	return(low);	/*found*/
		}
		low++;
    }
	...
	printf("NOT Found GPS map: mapset %d %s GPS %llu\n", 
		mapsetNow->sequence, mapsetNow->name, gps); 
	return(NULL);
}
...
*/
SDL_Surface* loadmap(	PMTmapset* mapsetNow, 
...
SDL_Point* gpstopixel( 	PMTmapindex* mapindexNow, 
						SDL_Surface* mapNow, 
						uint64_t gps )
{
...
/* returns (x + y) where x, y, and returned value are all mapkey format: ddmm  d=degrees, m=minutes */
...
uint64_t pixeltogps(PMTmapindex* mapindexNow, 
					SDL_Surface* mapNow, 
					SDL_Rect* camera)
{
	uint64_t	fakegps;
...
used in markwaypt.c, modifywaypt.c and createhike.c.    DISALLOW ON UTM, LCC MAPS!!!
Works for FES maps only. check before calling.   Just show start points for hikes on UTM maps. 
*/
