...
main() {
  ...
  if (initSDL2()) {
    ...
    err = SDL_UpdateTexture(globaltexture, NULL, removeme->pixels,
                            removeme->pitch);
    if (err != 0)
      printf("SDL_UpdateTexture Error %s\n", SDL_GetError());

    // Update the surface
    //		SDL_UpdateWindowSurface( globalwindow ); // THIS WORKS BY
    //ITSELF!!

    //  THESE 4 LINES ALSO WORK (REPLACES SINGLE LINE ABOVE) BUT IS SLOW AND
    //  FLAKY !!!!
