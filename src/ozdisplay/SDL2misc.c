...
int findmap(int flags, int mapindex)
{
	return(2);
}
...
SDL_Texture* gettexture( char* filename )
{
	...
	SDL_SetColorKey( temp, SDL_TRUE,
						 SDL_MapRGB( temp->format, 0xFF, 0xFF, 0xFF ) );
...
int incenter(int x, int y)
{
	...
	// SCREEN_WIDTH = center of screen = center of circle
	if( (x- SCREEN_WIDTH/2) * (x- SCREEN_WIDTH/2) + 
		(y- SCREEN_WIDTH/2) * (y- SCREEN_WIDTH/2) < 30*30 )
		return(TRUE);
	else
		return(FALSE);
}
...
double getangle( int x, int y, int xnew, int ynew)
{
	...
	clockwise = TRUE;
	if( x >= SCREEN_WIDTH/2 && y <= SCREEN_HEIGHT/2 &&	// 1st quadrant of circle
			xnew <= x  && ynew <= y )
		clockwise = FALSE;
	if( x <= SCREEN_WIDTH/2 && y <= SCREEN_HEIGHT/2 &&	// 2nd quadrant of circle
			xnew <= x  && ynew >= y )
		clockwise = FALSE;
	if( x <= SCREEN_WIDTH/2 && y >= SCREEN_HEIGHT/2 &&	// 3rd quadrant of circle
			xnew >= x  && ynew >= y )
		clockwise = FALSE;
	if( x >= SCREEN_WIDTH/2 && y >= SCREEN_HEIGHT/2 &&	// 4th quadrant of circle
			xnew >= x  && ynew <= y )
		clockwise = FALSE;
