...
/******************  display map  ***************************
   compile with 
gcc -g  displaymap.c SDL2init.c SDL2misc.c -o displaymap -lSDL2 -lSDL2_image -lSDL2_gfx -lm

RGB color scheme:  	stykman belt	= magenta (255,0,255)
					movemap arrows 	= yellow (255,255,0)
					newmap 			= orange (255,150,0)

FIXME todo someday when bored 
- indent is wrong 
- update all error handling	
	- change SDL calls to if( (x=SDL) !=0) SDLerr("display.c:12"); 
	- change PMT function calls: if( functioncall() != 0 ) PMTerr("display.c:13);
- figure out problem with alpha blending.... ??? and incorporate for max coolness
- when you define a pointer,  set it to NULL....  as in SDL_Surface peter = NULL;
...
/******************** Step 1 game initialize **************************/
	...
	if( (spritesheet = getsprite(filename)) == NULL)   exit(0);
    strcpy( filename, IMAGEPATH );
    strcat( filename, "buttonsheet.png" );
	if( (buttonsheet = getsprite(filename)) == NULL)   exit(0);
	...
	if(mymap == NULL)											exit(0);
	if((gpspixel = placegps(mymap) ) == NULL)  					exit(0);
...
/* SDL BUG:: FIXME ALPHA + SDL_RenderCopyEx() below causes greenring to disappear
	SDL_SetTextureBlendMode(greenring, SDL_BLENDMODE_BLEND); 
	...
	while( !quit )
	{
		...
		NOTE incircle is pass by value structure, NOT pass by reference */
						if( (mapflags & scalemapUPF 	&& 
							incircle( myfinger, nowest)  )				||

							(mapflags & scalemapDNF 	&& 
							incircle( myfinger, noeast) )				||

							(mapflags & movemapEF 	&& 
							incircle( myfinger, east) ) 				||

							(mapflags & movemapWF 	&& 
							incircle( myfinger, west) ) 				||

							(mapflags & movemapNF 	&& 
							incircle( myfinger, north)  )				||

							(mapflags & movemapSF 	&& 
							incircle( myfinger, south) ) )
...
*                          eg NPC Non-Player Character **********/
			...
			spriteframe = ((SDL_GetTicks() - startTime) * animationRate / 1000)
									 % spritecount;
...
/************ In Giant Loop: Step 4 Render to screen ****************/
			...
			err = SDL_UpdateTexture( globaltexture, NULL, 
				screenmap->pixels, screenmap->pitch);	
			if(err != 0)
			...
			if(spriteF)
				err = SDL_RenderCopy(globalrenderer, spritetexture, 
					&sprite[spriteframe], &spriteW); //sprite
			err = SDL_RenderCopy(globalrenderer, blackcircle,
					NULL,NULL); //screen outline

			/* Render outer rings if needed */ 
			if(pleaseorientF)
				err = SDL_RenderCopy(globalrenderer, pleaseorient,
					NULL, NULL); 
			if(ringF)
				err = SDL_RenderCopyEx(globalrenderer, greenring,
					NULL, NULL, angle, &screencenter, SDL_FLIP_NONE ); 

			/* render expanding center button */
			if( centerF )
	    		err = filledCircleRGBA( globalrenderer, 
					SCREEN_WIDTH/2, SCREEN_HEIGHT/2,			/* circle center */
					(SDL_GetTicks() - centerFmsecs)/BUTTONSPEED,/* circle radius*/ 
					207, 231, 229, 150 );			/* RGBA */

			/* render mapflag buttons for new maps?  */
			if( mapflags & scalemapUPF )
			{
/*				FIXME again (see line 181) alpha causes texture to disappear 
					when rendered....    alpha is problematic!!!
				err = SDL_SetTextureAlphaMod( buttontexture, alphadeleteme ); 	
	    		err = filledCircleRGBA( globalrenderer, 
					SCREEN_WIDTH/4, SCREEN_HEIGHT/4,	// circle center 
					30,	255, 0, 255, alphabutton );		// circle radius, RGBA 
*/

				buttonW.x = nowest.x - buttonW.w/2;
				buttonW.y = nowest.y - buttonW.h/2;				
				SDL_RenderCopy(globalrenderer, buttontexture, 
					&button[4], &buttonW); 
			}
